...
job_info() {
  local cnt=$(jobs -l | wc -l)

  if [ $cnt -gt 0 ]; then
    echo -n "%{$fg_no_bold[black]%}|%{$fg[yellow]%}$cnt%{$reset_color%}"
  fi
}
...
if [ -z "$DISPLAY" ] && [ -z "$SSH_CLIENT" ] && [ -z "$SSH_TTY" ]; then
	...
	HOST_PROMPT_="%{$fg_bold[black]%}=> %{$fg_no_bold[white]%}%{$fg_bold[black]%}[%{$fg_bold[cyan]%}%~%{$fg_bold[black]%}]%{$reset_color%}"
	GIT_PROMPT="\$(git_prompt_info)%{$reset_color%}"
	PROMPT="$HOST_PROMPT_$RUBY_PROMPT_$GIT_PROMPT$VENV_PROMPT_%{$fg_bold[black]%} $ %{$reset_color%}"
	RPROMPT="%{$reset_color%}%{$fg_bold[black]%}[%{$fg_no_bold[white]%}%*%(?..%{$fg_bold[black]%}|%{$fg[red]%}%?)%{$fg_bold[black]%}]%{$reset_color%}"
else 
	ZSH_THEME_GIT_PROMPT_DIRTY="%{$fg[green]%}) %{$fg[yellow]%}✗%{$reset_color%}"
	HOST_PROMPT_="%{$fg_no_bold[black]%}➜ %{$reset_color%}%{$fg[black]%}[%{$fg_no_bold[cyan]%}%~%{$fg[black]%}]%{$reset_color%}"
	GIT_PROMPT="\$(git_prompt_info)%{$reset_color%}"
	PROMPT="$HOST_PROMPT_$RUBY_PROMPT_$GIT_PROMPT$VENV_PROMPT_%{$fg_no_bold[black]%} $ %{$reset_color%}"
	RPROMPT="%{$reset_color%}%{$fg_no_bold[black]%}[%{$fg_no_bold[white]%}%*\$(job_info)%(?..%{$fg_no_bold[black]%}|%{$fg[red]%}%?)%{$fg_no_bold[black]%}]%{$reset_color%}"
fi
